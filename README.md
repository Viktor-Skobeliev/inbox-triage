...
`failed`, три відповіді довелося ремонтувати і вони пройшли з другої-третьої
спроби, дубль REQ-013 знайдено. Правила спіймали два місця, де вивід моделі
суперечив сам собі: REQ-012 позначено як `non_actionable` зі списком дій, а в
REQ-016 стоїть `needs_clarification` без жодного питання до автора. Але це один
...
Факт із прогону в `examples/`: 18 запитів, 23 виклики (18 розборів + 4 повторні
спроби + прохід по дублікатах), 31 217 токенів, з них 27 959 вхідних. Тобто
основна стаття витрат - інструкції, які повторюються в кожному запиті.
